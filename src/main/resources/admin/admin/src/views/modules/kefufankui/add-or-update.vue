...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.fankuineirong = this.ruleForm.fankuineirong.replace(reg,'../../../yinhangsystem/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `kefufankui/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.kefufankuiCrossAddOrUpdateFlag = false;
						 this.parent.search();
